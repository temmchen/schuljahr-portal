...
def schreibe_passwort_readme(cfg):
    """Lesbare Passwort-Übersicht in den OneDrive-Dashboard-Ordner schreiben.

    Liegt bewusst AUSSERHALB des Git-Repos (OneDrive, privat) und wird vom
    Build nie hochgeladen — build.py liest nur die Bereichsordner der Klassen.
    Wird bei jeder Änderung an zugangsdaten.json automatisch neu erzeugt."""
    try:
        ziel = Path(cfg["inhalt"]).expanduser() / "PASSWOERTER.md"
    except Exception:
        return
    if not ziel.parent.is_dir():
        return
    z = []
    z.append("# 🔐 Schuljahr-Portal — Zugänge & Passwörter")
    z.append("")
    z.append("> ⚠️ **NUR für dich (OneDrive, privat).** Niemals ins Portal-Repo kopieren,")
    z.append("> niemals öffentlich teilen. Einzelne Passwörter gibst du gezielt an die")
    z.append("> jeweilige Klasse bzw. den jeweiligen Kollegen weiter.")
    z.append(">")
    ...
    z.append("")
    z.append("## 🎓 Klassen (sehen nur Skripte der eigenen Klasse)")
    z.append("")
    z.append("| Klasse | Passwort | Zeitraum |")
    z.append("|---|---|---|")
    ...
    z.append("## 🛡️ Admin")
    z.append("")
    z.append(f"| **{a.get('name', 'Admin')}** | `{a.get('passwort', '?')}` |")
    z.append("|---|---|")
    z.append("")
    z.append("## Gut zu wissen")
    z.append("")
    z.append("- Passwort ändern: `python3 verwaltung.py passwort <Klasse|Name|admin>` "
             "(im Portal-Ordner) — diese Datei wird dabei automatisch neu geschrieben.")
    z.append("- Neue Klasse/Kollege: Ein-Klick-Button bzw. Admin-Panel im Portal "
             "oder `verwaltung.py klasse/prof`.")
    z.append("- Diese Datei liegt **nur** in deinem OneDrive — nicht im öffentlichen "
             "GitHub-Repo, und der Portal-Build lädt sie nie hoch.")
    ziel.write_text("\n".join(z) + "\n", encoding="utf-8")
