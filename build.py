...
BUILD_STATE = HIER / ".build-state.json"   # GEHEIM (gitignored): Vault-Schlüssel + Datei-IDs
                                           # → unveränderte Dateien werden NICHT neu verschlüsselt,
                                           #   damit die Git-Historie nicht bei jedem Build wächst.
...
def main():
    ...
    parser.add_argument("--neu-verschluesseln", action="store_true",
                        help="alle Tresore mit frischen Schlüsseln/IDs neu aufbauen "
                             "(echte Schlüsselrotation; lässt die Historie stark wachsen)")
    ...
    # ── Vaults bauen (inkrementell) ──
    # Unveränderte Dateien behalten ihre ID und ihr Chiffrat → Git sieht keine
    # Änderung → das Repo wächst nur um das, was wirklich neu ist.
    frisch = bool(getattr(args, "neu_verschluesseln", False))
    if frisch and VAULTS.exists():
        shutil.rmtree(VAULTS)
    VAULTS.mkdir(parents=True, exist_ok=True)

    def lade_build_state():
        if frisch:
            return {"vaults": {}, "dateien": {}}
        try:
            d = json.loads(BUILD_STATE.read_text(encoding="utf-8"))
            return {"vaults": d.get("vaults", {}), "dateien": d.get("dateien", {})}
        except Exception:
            return {"vaults": {}, "dateien": {}}

    alt_state = lade_build_state()
    neu_state = {"vaults": {}, "dateien": {}}
    zaehler = {"neu": 0, "wiederverwendet": 0}
    if frisch:
        print("🔄 Neuverschlüsselung: alle Tresore bekommen frische Schlüssel.\n")
    ...
    def baue_vault(klasse_cfg, bereich_typ, module_daten, berechtigte):
        ...
        vschluessel = f"{klasse_cfg['key']}|{bereich_typ}"
        vorher = alt_state["vaults"].get(vschluessel)
        if vorher:                                   # Tresor weiterverwenden
            vid = vorher["id"]
            k_vault = base64.b64decode(vorher["key"])
        else:
            vid = secrets.token_hex(8)
            k_vault = secrets.token_bytes(32)
        neu_state["vaults"][vschluessel] = {"id": vid, "key": b64(k_vault)}
        vdir = VAULTS / vid
            ...
            for b_key, eintraege in bereiche.items():
                ...
                for e in eintraege:
                    ...
                    unveraendert = (vorher_d
                                    ...
                                    and int(vorher_d["mtime"]) == int(st.st_mtime)
                    ...
                    if unveraendert:
                        ...
                        zaehler["neu"] += 1
                    neu_state["dateien"][dschluessel] = {
                        "fid": fid, "size": st.st_size, "mtime": int(st.st_mtime)}
                    behalten.add(f"{fid}.enc")
                    e["id"] = fid
                    e["groesse"] = st.st_size
                    gesamt += st.st_size
                    anzahl += 1
        ...
        manifest = {
            ...
            "bereich": bereich_typ,
            # "semester" (2 Zeiträume, DP-Klassen) oder "trimester" (3, GIG/GSO)
            "zeitraum": klasse_cfg.get("zeitraum", "semester"),
            # ESG-Klassen (1GIGF/2GIGF) haben nur EIN Référentiel (Programme),
            # nicht das DAP-Paar Formation + Évaluation.
            "ref_einzel": bool(klasse_cfg.get("ref_einzel", False)),
        ...
        for veraltet in (vdir / "f").iterdir():      # gelöschte Inhalte aufräumen
            ...
            for _sem, _mname, mk in modul_schluessel(fach):
                ...
                for p in sammle_dateien(mdir / REFERENTIELS_ORDNER):
                    eintrag = {"name": anzeige_name(p), "typ": p.suffix.lstrip(".").lower(),
                               "_pfad": str(p)}
                    nl = p.name.lower()
                    # erkennt auch eSchoolBooks-Codes: PGMFOR~… (Programme de formation), REFEVAL~…
                    # (identische Regel wie dashboard_server.py im OneDrive-Dashboard-Ordner)
                    if any(s in nl for s in ("formation", "pgmfor", "programme")) and not refs["formation"]:
                        refs["formation"] = eintrag
                    elif any(s in nl for s in ("evaluation", "évaluation", "refeval")) and not refs["evaluation"]:
                        refs["evaluation"] = eintrag
                    else:
                        refs["weitere"].append(eintrag)
                # Référentiels sind Lehrplan-Dokumente → sie liegen im SCHÜLER-Tresor,
                # damit Klassen sie ebenfalls öffnen können (Profs sehen ihn auch).
                skripte_module[mk] = {"skripte": sk, "referentiels": refs}
    ...
    # kekw: die KEKs aller Zugänge, AES-GCM-verschlüsselt unter dem ADMIN-KEK.
    # Nur der Admin (mit seinem Passwort) kann sie öffnen — damit kann die
    # Web-Verwaltung neue Klassen für alle Profs freischalten und Passwörter
    # rotieren, ohne dass je ein Schlüssel im Klartext im Repo liegt.
    admin_pr = next(p for p in principals if p["rolle"] == "admin")
    kekw = {}
    for p in principals:
        if p["id"] == admin_pr["id"]:
            continue
        kekw[p["id"]] = wickle_ein(admin_pr["kek"], {
            "kek": b64(p["kek"]), "rolle": p["rolle"],
    ...
    print(f"\nVerschlüsselt: {zaehler['neu']} Datei(en) neu, "
          ...
          f"(letztere lassen die Git-Historie NICHT wachsen).")

    # Tresore entfernen, die es nicht mehr gibt (z. B. gelöschte Klasse)
    aktuelle_ids = {v["id"] for v in index_vaults}
    for d in VAULTS.iterdir():
        if d.is_dir() and d.name not in aktuelle_ids:
            shutil.rmtree(d)
