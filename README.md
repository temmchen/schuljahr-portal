...
**Die Admin-Seite ist die Verwaltungszentrale** — direkt im Browser (auch iPad):
Dateien hochladen und löschen (🗑️), **neue Klassen anlegen** (inkl. Semester/
Trimester-Fächern und automatischer Prof-Freischaltung über den verschlüsselten
Admin-Schlüsselbund `kekw`), **Passwörter rotieren** (auch das eigene) und
**Zugänge entfernen**. Alles braucht die Admin-Rolle plus GitHub-Token; der
Schlüsselbund ist nur mit dem Admin-Passwort lesbar.
